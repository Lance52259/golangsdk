...
type ApiOpts struct {
	...
	// Chinese characters must be in UTF-8 or Unicode format.
	RespBody string `json:"body_remark,omitempty"`
...
type VpcChannel struct {
	// Proxy host.
	VpcChannelProxyHost string `json:"vpc_channel_proxy_host"`
	// VPC channel ID.
...
type PolicyWeb struct {
	...
	AuthorizerId string `json:"authorizer_id,omitempty"`
	// VPC channel details. This parameter is required if vpc_channel_status is set to 1.
	VpcChannelInfo VpcChannel `json:"vpc_channel_info"`
	// Indicates whether to use a VPC channel. The valid value are as following:
	//     1: A VPC channel is used.
	//     2: No VPC channel is used.
	VpcChannelStatus int `json:"vpc_channel_status"`
}
...
type ApiConditionBase struct {
	...
	ReqParamName    string  `json:"req_param_name"`
	// Policy condition.
	// exact: exact match
	// enum: enumeration
...
func Create(client *golangsdk.ServiceClient, instanceId, groupId string, opts ApiOptsBuilder) (r CreateResult) {
...
func SwitchSpecVersion(client *golangsdk.ServiceClient, instanceId, apiId string,
	...
	_, r.Err = client.Put(historyURL(client, instanceId, apiId), reqBody, &r.Body, nil)
	return
}
